...
// not a durable hard quota (see src/lib/x402.js for the design note, the
// rollover-prune that stops it leaking, and the KV path for a cross-isolate
// quota). `quotaState.lastPruneKey` records the month last pruned for.
const freeQuota = {};
const quotaState = { lastPruneKey: null };
function x402Gate(req, res, baseResource, priceAtomic, desc) {
  ...
  const hit = hitQuota(freeQuota, ip, monthKey(), FREE_LIMIT, quotaState);
  if (hit.allowed) { res.setHeader('X-Free-Calls-Remaining', String(hit.remaining)); return true; }
