...
export const SCORE_META = {
  ...
  inputCaveat: `Each board row carries volumeSource and feeSource: 'perChain' means that field is DefiLlama's authoritative per-chain figure; 'aggregate' means the per-chain call failed and the row kept the summed breakdown, which over-counts and reads 0 for any chain the feed names differently from the TVL feed. Chains outside the board are never enriched, so the lite index serves null rather than an aggregate that would read as a measurement. For chains on the board, the volume24h and fees24h fields in this payload ARE the inputs the score was computed from: DefiLlama's authoritative per-chain figures (volume is spot DEX categories only — derivatives, prediction markets, NFT marketplaces and aggregators are excluded). Score is therefore reproducible from this payload. Chains outside the board are ranked on a provisional score built from the summed per-protocol breakdowns, which under-reports any chain the DEX or fee feed names differently from the TVL feed.`,
};
