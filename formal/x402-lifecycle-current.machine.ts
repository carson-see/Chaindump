...
export const x402Lifecycle = defineMachine({
  version: 2,
  moduleName: "X402LifecycleCurrent",
  variables: {
  ...
  actions: {
    ...
    // BUG UNDER TEST: current merged ordering settles straight from `verified`,
    // BEFORE serving — so a 404 after this leaves the agent charged, unserved.
    settleOk: {
      params: { r: "Reqs" },
      guard: eq(index(phase, param("r")), lit("verified")),
