...
function cgUrl(url) { return CG_KEY ? url + (url.includes('?') ? '&' : '?') + 'x_cg_demo_api_key=' + encodeURIComponent(CG_KEY) : url; }
...
const DASH = process.env.DASHBOARD_BASE || `http://localhost:${process.env.DASHBOARD_PORT || 4000}`;
const DTOKEN = process.env.DASHBOARD_TOKEN;
...
app.get('/api/chain/:name', async (req, res) => {
  try {
...
app.get('/api/dead', async (req, res) => {
  try {
...
app.get('/api/uspolicy', async (req, res) => {
  try {
...
const X402 = {
  payTo: process.env.X402_PAY_TO || '0xee321Ac2315e6b60c2dEE4E989767C79b73e6f0d',
  network: process.env.X402_NETWORK || 'base',
  asset: process.env.X402_ASSET || '0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913', // USDC on Base
  facilitator: process.env.X402_FACILITATOR || 'coinbase-cdp',
};
...
app.get('/api/agent/chain/:key', async (req, res) => {
...
app.get('/api/agent/risk/:entity', async (req, res) => {
...
app.get('/api/trace-lookup', async (req, res) => {
...
app.get('/api/scam-graph', async (req, res) => {
  try {
...
app.listen(PORT, () => console.log(`chain-monitor listening on ${PORT}`));
